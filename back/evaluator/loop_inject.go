...
func GenerateTickFunction(inject *LoopInject, unitLocation *location.UnitLocation, output OutputDirectory) (module, unit, name string) {
	loopFunctionName := fmt.Sprintf("loop-%s", uuid.New().String())
	loopFunctionBody := inject.InjectBody
	DumpFunction(unitLocation.Module, unitLocation.Unit, loopFunctionName, loopFunctionBody, output)
...
func GenerateDelayLines(inject *LoopInject, delay int, functionCall *ast.FunctionCall, data *EvaluateData) (runCommand string) {
	...
	summonCloud := fmt.Sprintf("execute at @p run summon minecraft:area_effect_cloud ~ ~ ~ {Tags:[\"%s\"],Duration:%d}", cloudID, delay+1)
	return summonCloud
}
